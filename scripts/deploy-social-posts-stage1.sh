...
echo "== migrate social posts stage 1 =="
DBURL=$(node -e "const m=require('./ecosystem.config.cjs'); console.log(m.apps[0].env.DATABASE_URL)")
psql "$DBURL" -v ON_ERROR_STOP=1 -f scripts/migrate-social-posts-stage1.sql

echo "== ensure ORDERLY_SOCIAL_POSTING_ENABLED present (default off) =="
node <<'NODE'
const fs = require('fs');
const p = '/var/www/samurai-resto/ecosystem.config.cjs';
let src = fs.readFileSync(p, 'utf8');
if (!/ORDERLY_SOCIAL_POSTING_ENABLED/.test(src)) {
  src = src.replace(
    /(env\s*:\s*\{)/,
    `$1\n      ORDERLY_SOCIAL_POSTING_ENABLED: "0",`,
  );
  fs.writeFileSync(p, src);
  console.log('added ORDERLY_SOCIAL_POSTING_ENABLED=0');
} else {
  console.log('ORDERLY_SOCIAL_POSTING_ENABLED already present');
}
NODE
...
echo "Done. Stage 1 = manual post only. No Graph auto-publish."
